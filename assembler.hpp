...
const std::map<std::string, InstructionCodes> INSTR_CODES = {
    ...
    {"srl", {0x00, INSTR_TYPE_R_SHIFT, 0x02}},  // R, 0x02
    {"sra", {0x00, INSTR_TYPE_R_SHIFT, 0x03}},  // R, 0x03
...
int assemble(const std::string& finput, const std::string& foutput_listing, const std::string& foutput_instr) {
    // open files
    std::ifstream fileReader(finput);
    std::ofstream outputListing(foutput_listing);
    std::ofstream outputInstructions(foutput_instr);
    if(!fileReader.good() || !outputInstructions.good() || ! outputListing.good()){
